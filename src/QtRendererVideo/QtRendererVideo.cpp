...
void QtRendererVideo::InitializeGL()
{
	...
	else if (capture->formatType == PIX_FMT_YUYV422)
	{
		shader = new Shader("assets/vertexShader.glsl", "assets/VideoShader/fragmentShader_yuyv422.glsl");
		tex1 = new Texture2D(capture->width, capture->height, GL_LUMINANCE_ALPHA, GL_LUMINANCE_ALPHA, NULL);

		tex2 = new Texture2D(capture->width / 2, capture->height, GL_RGBA, GL_RGBA, NULL);
	}
	else if (capture->formatType == PIX_FMT_UYVY422)
	{
		shader = new Shader("assets/vertexShader.glsl", "assets/VideoShader/fragmentShader_uyvy422.glsl");
		tex1 = new Texture2D(capture->width, capture->height, GL_LUMINANCE_ALPHA, GL_LUMINANCE_ALPHA, NULL);
	...
	else if (capture->formatType == PIX_FMT_GRAY)
	{
		shader = new Shader("assets/vertexShader.glsl", "assets/VideoShader/fragmentShader_gray.glsl");
		tex1 = new Texture2D(capture->width, capture->height, GL_LUMINANCE, GL_LUMINANCE, NULL);
	}
	else if (capture->formatType == PIX_FMT_RGB)
	{
		shader = new Shader("assets/vertexShader.glsl", "assets/VideoShader/fragmentShader_rgb.glsl");
		tex1 = new Texture2D(capture->width, capture->height, GL_RGB, GL_RGB, NULL);
	}
	else if (capture->formatType == PIX_FMT_BGR)
	{
...
retry:
	...
	else if (capture->formatType == PIX_FMT_YUYV422 || capture->formatType == PIX_FMT_UYVY422)
	{
